...
fn main() {
    let xml = include_str!("../cameras.xml");

    let parsed_data: Cameras = serde_xml_rs::from_str(xml).unwrap();
    let existing_cameras = parse_cameras()
        .into_iter()
        .map(|item| ((item.clean_make.clone(), item.clean_model.clone()), item))
        .collect::<HashMap<_, _>>();

    let parsed_cameras = parsed_data
        .group_by_make_and_model()
        .into_iter()
        .filter_map(|((make, model), mut modes)| {
            modes.retain(|item| item.supported != Some("no".to_string()));
            ...
            Some(((camera.make.clone(), camera.model.clone()), camera))
        })
        .collect::<HashMap<_, _>>();

    println!(
        "Existing Cameras: {} | Parsed Cameras: {}",
        existing_cameras.len(),
        parsed_cameras.len()
    );
}
