...
impl<'a> TiffIFD<'a> {
    ...
    pub fn find_entry(&self, tag: Tag) -> Option<&TiffEntry<'_>> {
        if self.entries.contains_key(&t(tag)) {
            self.entries.get(&t(tag))
        } else {
            for ifd in &self.subifds {
                if let Some(x) = ifd.find_entry(tag) { return Some(x) }
            }
            None
        }
    }
    ...
    pub fn find_ifds_with_tag(&self, tag: Tag) -> Vec<&TiffIFD<'_>> {
    ...
    pub fn find_first_ifd(&self, tag: Tag) -> Option<&TiffIFD<'_>> {
