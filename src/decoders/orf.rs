...
impl<'a> Decoder for OrfDecoder<'a> {
    fn image(&self, dummy: bool) -> Result<RawImage, String> {
        ...
        for i in 0..counts.count() {
            size += counts.get_u32(i) as usize;
        }
...
impl<'a> OrfDecoder<'a> {
    ...
    pub fn decode_compressed(buf: &'a [u8], width: usize, height: usize, dummy: bool) -> Vec<u16> {
        ...
        for (i, item) in bittable.iter_mut().enumerate() {
            let mut b = 12;
            for high in 0..12 {
                if ((i >> (11 - high)) & 1) != 0 {
                    b = high;
    ...
    fn get_wb(&self) -> Result<[f32; 4], String> {
        ...
        if let (Some(redmul), Some(bluemul)) = (redmul, bluemul) {
            Ok([
                redmul.get_u32(0) as f32,
                256.0,
                bluemul.get_u32(0) as f32,
                f32::NAN,
            ])
        } else {
            ...
            Ok([wbs.get_f32(0), 256.0, wbs.get_f32(1), f32::NAN])
        }
    }
}
