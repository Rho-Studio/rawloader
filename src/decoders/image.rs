...
impl RawImage {
    ...
    pub fn neutralwb(&self) -> [f32; 4] {
        ...
        for (i, cam_value) in rgb_to_cam.iter_mut().enumerate() {
            for j in 0..3 {
                cam_value[j] = 0.0;

                for (k, xyz_value) in rgb_to_xyz.iter().enumerate() {
                    cam_value[j] += self.xyz_to_cam[i][k] * xyz_value[j];
                }
            }
        }
    ...
    pub fn normalized_pseudoinverse(inm: [[f32; 3]; 4]) -> [[f32; 4]; 3] {
        ...
        for item in &mut xyz_to_cam {
            let mut num = 0.0;

            for inner in item.iter().take(3) {
                num += inner;
            }

            for inner in item.iter_mut().take(3) {
                *inner = if num == 0.0 { 0.0 } else { *inner / num };
            }
        }
    ...
    pub fn pseudoinverse(inm: [[f32; 3]; 4]) -> [[f32; 4]; 3] {
        ...
        for (i, item) in inm.iter().enumerate() {
            for j in 0..3 {
                out[j][i] = 0.0;

                for (k, inner) in item.iter().enumerate().take(3) {
                    out[j][i] += temp[j][k + 3] * inner;
                }
            }
        }
