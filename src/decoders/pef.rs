...
impl<'a> PefDecoder<'a> {
    ...
    fn get_blacklevels(&self) -> Option<[u16; 4]> {
        self.tiff.find_entry(Tag::PefBlackLevels).map(|levels| {
            [
    ...
    pub(crate) fn do_decode(
        ...
        if let Some((huff, endian)) = huff {
            ...
            for item in v0.iter_mut().take(depth) {
                *item = stream.get_u16() as u32;
            }

            let mut v1: [u32; 16] = [0; 16];
            for item in v1.iter_mut().take(depth) {
                *item = stream.get_u8() as u32;
            }
            ...
            for i in 0..depth {
                ...
                for (j, item) in v2.iter().enumerate().take(depth) {
                    if *item <= sm_val {
                        sm_num = j as u32;
                        sm_val = *item;
                    }
            ...
            for (i, item) in pentax_tree.iter().enumerate().take(16) {
                htable.bits[i + 1] = *item as u32;
